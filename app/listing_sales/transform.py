...
def validate_money_field(value, field_name):
    ...
    try:
        ...
        limits = {
            'sale_price': 100000000,  # 100 million
        }
...
def transform_listings_sales(listings_sales_table, load_date):
    ...
    try:
        # Load the schema
        schema = load_schema(LISTING_SALE_SCHEMA_PATH)
        # Preview the input table
        preview_table(listings_sales_table, message="Input listings sales table:")

        # Conver column types based on the schema
        listings_sales_table = convert_column_types(listings_sales_table, schema)

        # Convert PETL table to pandas DataFrame
        listings_df = etl.todataframe(listings_sales_table)

        # Mandatory Keys
        mandatory_keys = ['listing_sale_id', 'organization_id', 'user_id']
        ...
        date_fields = ['created_at', 'updated_at', 'deleted_at', 'load_date']
        ...
        logger.exception("Error transforming listings sales data")
        raise
